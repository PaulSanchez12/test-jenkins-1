...
pipeline {
 ...
 stages {
        stage('Checkout-git'){
               steps{
		git poll: true, url: 'git@github.com:PaulSanchez12/test-jenkins-1.git'
               }
        }
    ...
    stage('PushDockerImage') {
            steps {
            	sh '''
            		docker tag apptest:latest psanchez12/apptest:latest
					docker push psanchez12/apptest:latest
